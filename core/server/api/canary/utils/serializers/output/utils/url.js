...
//hack
const forPostWithData = (data, attrs, frame) => {
    //hack
    // console.log("hack - Post data is: ", data);
    attrs.url = urlService.getUrlByResource(data, "posts", {absolute: true});
...
//hack
const forUserWithData = (data, attrs, options) => {
    if (!options.columns || (options.columns && options.columns.includes('url'))) {
        attrs.url = urlService.getUrlByResource(data, "users", {absolute: true});
    }
...
const forTagWithData = (data, attrs, options) => {
    if (!options.columns || (options.columns && options.columns.includes('url'))) {
        attrs.url = urlService.getUrlByResource(data, "tags", {absolute: true});
    }
...
module.exports.forPostWithData = forPostWithData;
module.exports.forUserWithData = forUserWithData;
module.exports.forTagWithData = forTagWithData;
